...
class ImageProcessingNode(Node):
    def __init__(self):
        ...
        self.pub_tracked_point = self.create_publisher(Float64MultiArray, 'bluerov2/tracked_point', 10)
        self.pub_desired_point = self.create_publisher(Float64MultiArray, 'bluerov2/desired_point', 10)
        self.pub_radius_buoy = self.create_publisher(Float64, 'bluerov2/radius_buoy', 10)
        
        self.subscription = self.create_subscription(
            Image,                    # Message type
            'camera/image',  # Topic (assumed topic name)
            # 'webcam/image_raw',  # Topic (assumed topic name)
    ...
    def cameracallback(self, msg):
        ...
        global get_hsv, set_desired_point, desired_point, mouseX, mouseY, hsv_value, is_initial_desired_point, lx
        ...
        if is_initial_desired_point:
            desired_point = [image_width/2,image_height/2]
            is_initial_desired_point = False
        ...
        if set_desired_point:
            ...
            self.get_logger().info(f"Desired Value at ({mouseX}, {mouseY}): {desired_point}")
        ...
        if contours:
            # Find the largest contour (optional, if multiple)
            ...
            if radius > 5:  # Ignore tiny noise
                center = (int(x), int(y))
                radius = int(radius)
                self.radius_meters = radius / lx
                cv2.circle(image_np, center, radius, (0, 0, 255), 2)  # Red circle
            else:   # No significant contour found -> area zero
                self.radius_meters = 0.0

        # publish area value
        radius_msg = Float64()
        radius_msg.data = self.radius_meters
        self.pub_radius_buoy.publish(radius_msg)
